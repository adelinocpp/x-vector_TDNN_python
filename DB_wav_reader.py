...
def get_num_class(folderName):
    file_list = find_feats(folderName)
    label = np.array([]);
    for idx, filename in enumerate(file_list):
        filenameParts = filename.replace('\\', '/')
        filenameFolder = int(filenameParts.split('/')[-2]) % 10000 # manten os quatro ultimos indices
...
def find_save_models(directory, pattern='**/*.pth'):
    """Recursively finds all files matching the pattern."""
    return glob(os.path.join(directory, pattern), recursive=True)
...
def find_xvector(directory, pattern='*.pth'):
    """Recursively finds all files matching the pattern."""
    return glob(os.path.join(directory, pattern), recursive=True)
...
#     DB_dir = '../../00_Bases_de_Dados/98_CEFALA1_Tratada/CEFALA_8k_Refinados'
